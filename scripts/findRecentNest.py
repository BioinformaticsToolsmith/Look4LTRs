...
def find_nested(lines, visited, x, graph_id, data, parent):
    if data[6] != "NA" and data[13] == "RecentlyNested":
        for nested in data[6][1:-1].split(',')[:-1]:
            for y in range(x + 1, len(lines)):
                nest_data = lines[y].split()
                if nest_data[1] == nested and nest_data[14] == graph_id and nest_data[4] != "NA" and nest_data[13] == "RecentlyNested":
                    visited.add(y)
    ...
    with open(file, 'r') as f:
        ...
        for x in range(1, len(lines)):
            if x in visited:
                continue
            data = lines[x].split()
            if data[4] != "NA" and data[13] == "RecentlyNested":
                graph_id = data[14]
