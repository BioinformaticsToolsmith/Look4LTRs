...
I KmerHashTable<I, V>::hash(const char * sequence, int keyStart) {
	...
	for (int i = 0; i < k; i++) {
		...
		if (nucleotide >= 0 && nucleotide <= 3) {
			...
			msg.append(utility::Util::int2string((int) nucleotide));
			msg.append(" of nucleotide at index ");
			msg.append(utility::Util::int2string(keyStart + i));
...
void KmerHashTable<I, V>::hash(const char * sequence, int start, int end,
	...
	for (int i = start; i <= end; i++) {
		...
		if (!(nucleotide >= 0 && nucleotide <= 3)) {
			...
			msg.append(utility::Util::int2string((int) nucleotide));
			msg.append(" of nucleotide at index ");
			msg.append(utility::Util::int2string(i));
...
/**
 * Divide the size of the table into roughly equal segment,
 * each of which is divisible by 4 and will be processed by
 * a separate core.
 * Return: <segLength, segCount> 
 */
template<class I, class V>
std::pair<I,I> KmerHashTable<I, V>::getSegmentLengthAndCount(){
	I segLength = ceil(KmerHashTable<I, V>::maxTableSize / (double) utility::Util::CORE_NUM);
	
	if(segLength % 4 != 0){
		segLength = segLength + 4 - (segLength % 4 );
	}

	if(segLength % 4 != 0){
		cerr << "The table segment must be divisable by 4.";
		cerr << endl;
		throw std::exception();
	}
	I segCount = ceil(KmerHashTable<I, V>::maxTableSize / (long double) segLength);
	return std::make_pair(segLength, segCount);
}
