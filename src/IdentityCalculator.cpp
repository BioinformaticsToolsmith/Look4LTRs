...
// template <typename V>
// IdentityCalculator<V> IdentityCalculator<V>::makeInstance(DataGenerator *g, int threadNum, double t, bool skip, bool relax, std::string modelFile)
// {
// 	return new IdentityCalculator<V>(g, threadNum, t, skip, relax, modelFile);
// }
...
/**
 * One sequence versus one sequence
 */
template <class V>
double IdentityCalculator<V>::score(std::string *seq1, std::string *seq2)
{
	V *kHist1 = kTable->build(seq1);
	uint64_t *monoHist1 = monoTable->build(seq1);
	V *kHist2 = kTable->build(seq2);
	uint64_t *monoHist2 = monoTable->build(seq2);

	int len1 = seq1->size();
	int len2 = seq2->size();

	double r = score(kHist1, kHist2, monoHist1, monoHist2, calcRatio(len1, len2), len1, len2);
...
#pragma omp parallel for schedule(static) num_threads(threadNum)
	for (int i = 0; i < size; i++)
	{
		...
		// A check
		if (Util::isAllZeros(kHistList[i], kHistSize) || Util::isAllZeros(monoHistList[i], monoHistSize))
		{
