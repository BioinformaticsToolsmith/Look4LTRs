...
GlobAlignE::GlobAlignE(const char *seq1In, int start1In, int end1In,
		const char *seq2In, int start2In, int end2In, int matchIn,
		int mismatchIn, int gapOpenIn, int gapContinueIn) {
...
void GlobAlignE::findAlignment() {
	...
	alignmentScore = max(max(matches[len1 - 1], lowerGap[len1 - 1]),
			upperGap[len1 - 1]);
