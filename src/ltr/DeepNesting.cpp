...
void DeepNesting::findRegion() {
    for (auto rt : *rtVecPtr) {
        // Look for the elements who were nested outside!
        bool isRecentOuter = !(rt->getCaseType().compare("RecentlyNestedOuter"));
        bool hasNested = rt->hasRightLTR()? !rt->getNestSet().empty() : false;
        if (isRecentOuter && hasNested) {

            // Look through the outer element's nests
            for (auto nest : rt->getNestSet()) {

                // Searching for the recently nested inside element
                if (nest->getCaseType() == "RecentlyNestedInner") {
                    ...
                    int nestStart = nest->getStart() - graphRegion.first;
                    // How long is the nested element?
                    int length = nest->getSize();
...
std::vector<ModulePipeline*> DeepNesting::findDeep(std::string &graphSeq, int removeStart, int removeLength, int level) {
    ...
    ModulePipeline *mp = new ModulePipeline{red, &cutSeq};
    std::vector<ModulePipeline*> mpVec{mp};

    mp->buildElements();
    mp->matchElements(icStandard, icRecent);
    mp->process(icStandard, true);
    auto rtVec = mp->getRtVec();
    std::vector<RT*> r;

    for (auto &rt : *rtVec) {
        if (rt->getCaseType() == "RecentlyNestedOuter" && !rt->getNestSet().empty()) {
            for (auto nest : rt->getNestSet()) {
                ...
                if (nest->getCaseType() == "RecentlyNestedInner") {
                    ...
                    // }
                    auto nestModulePipeline = findDeep(cutSeq, nest->getStart(), nest->getSize(), level + 1);
