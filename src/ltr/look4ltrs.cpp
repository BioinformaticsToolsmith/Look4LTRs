...
int main(int argc, char*argv[]) {
    ...
    */
    ...
    try {
        ...
        red = std::make_unique<Red>(redTrainPath);
    }
    ...
    for(int i = 0; i < fastaVec.size(); i++) {
        ...
        for (auto &chrom : *fr.read()) {
            ...
            #pragma omp critical 
            {
                moduleMap[chromOut] = new ModulePipeline{*red, chrom.second};
            }
    ...
    for (int i = 0; i < fastaVec.size(); i++) {
        ...
        for (auto &chrom : *fr.read()) {
            ...
            ModulePipeline &mp = *moduleMap[chromOut];
            mp.writeToDB(dbPath, i, chromName);
        }
    }
    ...
    for(int i = 0; i < predVec.size(); i++) {
        ...
        for (auto &chrom : *fr.read()) {
            ...
            mp.findDeepNests(*icStandard, *icRecent);
