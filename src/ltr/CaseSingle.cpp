...
bool CaseSingle::checkSingle(std::vector<int> &leftLtrScoreVec, std::vector<int> &interiorScoreVec, std::vector<int> &rightLtrScoreVec) {
    ...
    double leftRatio = leftLtrScoreMedian / interiorScoreMedian;
    double rightRatio = rightLtrScoreMedian / interiorScoreMedian;
    double interiorPerc = LtrUtility::calcPercent(interiorScoreVec);

    r = leftRatio >= LtrParameters::MIN_LTR_INT_RATIO && rightRatio >= LtrParameters::MIN_LTR_INT_RATIO && interiorPerc >= LtrParameters::MIN_PERC;
