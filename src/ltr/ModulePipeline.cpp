...
ModulePipeline::ModulePipeline(Red &_red, std::string *_chromosome) : red(_red)
{
...
void ModulePipeline::buildElements()
{
    // Score the sequence
    // std::cout << "Scoring the sequence..." << std::endl;
    ScorerTr st(*chromosome, LtrParameters::K, LtrParameters::MIN_DISTANCE, LtrParameters::MAX_DISTANCE);
...
void ModulePipeline::writeToDBHelper(std::string filePath, std::vector<Element> & eleVec, int fastaID, std::string chromName) {
...
void ModulePipeline::writeToDB(std::string filePath, int fastaID, std::string chromName) {
    writeToDBHelper(filePath, fElement, fastaID, chromName);
    writeToDBHelper(filePath, bElement, fastaID, chromName);
}
...
void ModulePipeline::matchElements(IdentityCalculator<int32_t> &icStandard, IdentityCalculator<int32_t> &icRecent, std::string *_chromosome ) {
...
void ModulePipeline::findDeepNests(IdentityCalculator<int32_t> &icStandard, IdentityCalculator<int32_t> &icRecent) {
    assert(mat != nullptr);
    process(icStandard,  true);
...
void ModulePipeline::process(IdentityCalculator<int32_t> &icStandard, bool nestOnly) {
    PostProcess post{*mat->getRtVec(), *mat->getComplexVec(), chromosome->size(), icStandard, chromosome};
    post.apply(nestOnly);
}

void ModulePipeline::filter() {
