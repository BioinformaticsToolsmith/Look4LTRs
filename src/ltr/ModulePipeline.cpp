...
void ModulePipeline::buildStretches(std::string *chromosome)
{ 
    // Score the sequence
...
void ModulePipeline::writeFeatureMatrix(std::string filePath, std::string *chromosome, std::string *otherChromosome) {
    std::ofstream featureFile{filePath};
    Detector dt{red, *chromosome, *otherChromosome};

    Matrix fFeatures = dt.buildMatrix(*forwardMerger->getStretchVec());

    for (int i = 0; i < fFeatures.getNumRow(); i++) {
        for (int j = 0; j < fFeatures.getNumCol() - 1; j++) {
            featureFile << fFeatures(i, j) << ",";
        }
        featureFile << fFeatures(i, fFeatures.getNumCol() - 1) << std::endl;
    }

    Matrix bFeatures = dt.buildMatrix(*backwardMerger->getStretchVec());

    for (int i = 0; i < bFeatures.getNumRow(); i++) {
        for (int j = 0; j < bFeatures.getNumCol() - 1; j++) {
            featureFile << bFeatures(i, j) << ",";
        }
        featureFile << bFeatures(i, bFeatures.getNumCol() - 1) << std::endl;
    }

    featureFile.close();
}

void ModulePipeline::writeStretches(std::string forwardFilePath, std::string backwardFilePath) {
    std::ofstream forwardStretchFile{forwardFilePath};
    std::ofstream backwardStretchFile{backwardFilePath};

    for (auto &s : *forwardMerger->getStretchVec()) {
        forwardStretchFile << s.getStart() << "," << s.getEnd() << "," << s.getMedianHeight() << std::endl;
    }
    for (auto &s : *backwardMerger->getStretchVec()) {
        backwardStretchFile << s.getStart() << "," << s.getEnd() << "," << s.getMedianHeight() << std::endl;
    }

    forwardStretchFile.close();
    backwardStretchFile.close();
}
...
std::vector<Stretch> * ModulePipeline::getStretchVecPtr(bool isForward) {
    return isForward? forwardMerger->getStretchVec():backwardMerger->getStretchVec();
}

DirectedGraph<Element>* ModulePipeline::getGraph() const {
    return mat->getGraph();
}
