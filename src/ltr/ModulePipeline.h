...
private:
    ...
    // Methods
    void writeToDBHelper(std::string filePath, std::vector<Element> & eleVec, int fastaID, std::string chromName);
...
public:
    ...
    ModulePipeline(Red &_red, std::string *_chromosome);
    ~ModulePipeline();
    ...
    // Methods
    void buildElements();
    void writeToDB(std::string filePath,  int fastaID, std::string chromName);
    void writeElements(std::string filePath);
    void clearElementVec();

    void readElements();
    void removeElements();
    void matchElements(IdentityCalculator<int32_t> &icStandard, IdentityCalculator<int32_t> &icRecent, std::string *_chromosome = nullptr);
    void findDeepNests(IdentityCalculator<int32_t> &icStandard, IdentityCalculator<int32_t> &icRecent);
    void process(IdentityCalculator<int32_t> &icStandard, bool nestOnly = false);
    void filter();
};