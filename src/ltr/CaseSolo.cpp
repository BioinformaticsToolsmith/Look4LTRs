                ...
                if (graph.isBidirectional(*fElePtr, *bElePtr) && isReciprocal(fElePtr, bElePtr, graph))
                {
                    ...
                    if (vert != nullptr)
                    {
                        for (auto finalElePtr : getDiagonals(vert, graph))
                        {
                            ...
                            {
                                Element middleLtr{*bElePtr, *vert};
                                ...
                                if (fElePtr->getEnd() <= middleLtr.getStart() && middleLtr.getEnd() <= finalElePtr->getStart())
                                {
                                    ...
                                    if (isLeft)
                                    {
                                        rtVec.push_back(new RTComplete{new Element{*fElePtr}, new Element{middleLtr}, name + "Single", rank, graphIndex});
                                    ...
                                    if (isRight)
                                    {
                                        ...
                                        rtVec.push_back(new RTComplete{new Element{middleLtr}, new Element{*finalElePtr}, name + "Single", rank, graphIndex});
                                    }
                                    if (isAll)
                                    {
                                        rtVec.push_back(new RTComplete{new Element{*fElePtr}, new Element{*finalElePtr}, name + "Single", rank, graphIndex});
                                        rtVec.push_back(new RTSolo{new Element{middleLtr}, name + "LTR", rank, graphIndex});
                                    }
